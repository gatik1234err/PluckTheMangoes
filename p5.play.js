...
/**
   ...
   * an element able to store sprites or animations with a set of
...
function Sprite(pInst, _x, _y, _w, _h) {
  ...
  * 0.5 will be half. Scaling up may make sprites blurry.
  *
  * @property scale
  * @type {Number}
  * @default 1
  ...
  * If no sprites or animations are set it's the width of the
  ...
  * If no sprites or animations are set it's the height of the
  * placeholder rectangle.
  * Used internally to make calculations and draw the sprite.
  *
  * @private
  ...
  * If 1 the sprites displayed normally
  * If -1 the sprites are flipped horizontally
  ...
  * If 1 the sprites displayed normally
  * If -1 the sprites are flipped vertically
  ...
  */
  ...
  this.play = function() {
    ...
    if (!this.animation.looping && !this.animation.playing && this.animation.getFrame() === this.animation.sprites.length - 1) {
  ...
  */
  this.addAnimation = function(label)
  {
    ...
    {
      print('addAnimation error: you must specify a label and n frame sprites');
      return -1;
    }
...
/**
 * An Animation object contains a series of sprites (p5.Image) that
 * can be displayed sequentially.
 *
 * All files must be png sprites. You must include the directory from the sketch root,
 ...
 * "data/file0001.png" and "data/file0005.png" the sprites
...
function Animation(pInst) {
  ...
  */
  ...
  if(frameArguments.length === 2 && typeof frameArguments[0] === 'string' && typeof frameArguments[1] === 'string')
  {
    ...
    if(from !== -1 && to !== -1)
    {
      ...
      if(prefix1 !== prefix2 )
      {
        //print("2 separate sprites");
        this.sprites.push(pInst.loadImage(from));
        this.sprites.push(pInst.loadImage(to));
      }
      ...
      else
      {
        ...
        if(digits1 === digits2)
        {
          ...
          for (i = number1; i <= number2; i++) {
            ...
            this.sprites.push(pInst.loadImage(fileName));
  ...
  else if (frameArguments.length === 1 && (frameArguments[0] instanceof SpriteSheet))
  {
    ...
    this.sprites = this.spriteSheet.frames.map( function(f) {
  ...
  else if(frameArguments.length !== 0)//arbitrary list of sprites
  {
    ...
    for (i = 0; i < frameArguments.length; i++) {
      ...
      if(frameArguments[i] instanceof p5.Image)
        this.sprites.push(frameArguments[i]);
      else
        this.sprites.push(pInst.loadImage(frameArguments[i]));
    }
  }
  ...
  */
  this.clone = function() {
    ...
    myClone.sprites = this.sprites.slice();
  ...
  this.update = function() {
    ...
    if ( this.playing && cycles%this.frameDelay === 0)
    {
      ...
      else if (this.looping) //advance frame
      {
        ...
        if (frame>=this.sprites.length-1)
          frame = 0;
        else
          frame++;
      } else
  ...
  */
  this.changeFrame = function(f) {
    if (f<this.sprites.length)
      frame = f;
    else
      frame = this.sprites.length - 1;
  ...
  */
  this.goToFrame = function(toFrame) {
    if(toFrame < 0 || toFrame >= this.sprites.length) {
      return;
    }
  ...
  */
  this.getWidth = function() {
    if (this.sprites[frame]) {
      return this.sprites[frame].sourceW || this.sprites[frame].width;
    } else {
      return 1;
    }
  };
  ...
  */
  this.getHeight = function() {
    if (this.sprites[frame]) {
      return this.sprites[frame].sourceH || this.sprites[frame].height;
    } else {
      return 1;
    }
  };
